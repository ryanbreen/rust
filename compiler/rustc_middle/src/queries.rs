...
rustc_queries! {
    ...
    /// constructors. Unlike `is_mir_available`, this query will not answer `true` for the generated
    /// by-move MIR bodies of coroutine closures.
    ...
    query is_mir_for_hir_available(key: DefId) -> bool {
        desc { |tcx| "checking if HIR item has MIR available: `{}`", tcx.def_path_str(key) }
        cache_on_disk_if { key.is_local() }
        separate_provide_extern
    }

    /// Answers whether MIR is available for the given `key`. Unlike `is_mir_for_hir_available`,
